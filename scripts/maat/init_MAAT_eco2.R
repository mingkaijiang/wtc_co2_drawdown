...
canopy.fnames.static <- list(
  ...
  lai = 'f_lai_constant',     
  scale_vcmax = 'f_scale_two_layer'     
)   

canopy.pars.static   <- list(
  vcmax = list(               # vcmax:
    layer1 = 97,              #   in first canopy layer
    layer2 = 45.4             #   in second canopy layer
  ),
  jmax = list(                # jmax:
    layer1 = 169,             #   in first canopy layer
    layer2 = 82.3             #   in second canopy layer
  ),
  f = list(                   # f:
    layer1 = 0.51,            #   in first canopy layer
    layer2 = 0.79             #   in second canopy layer
  ),
  g1 = list(                  # g1 (medlyn):
    layer1 = 3.45,            #   in first canopy layer
    layer2 = 5.40             #   in second canopy layer
  )
)

canopy.env.static    <- list(
  vpd = 1.12
)  
...
canopy.env.var       <- list(
  par     = c(seq(50,2000,50), 1706, 1620 ),
  ca_conc = c(400, 600 )
)
