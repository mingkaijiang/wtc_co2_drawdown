compare_chamber_results_at_canopy_level <- function() {
    ...
    ### calculate sun leaf to shaded leaf contribution to total A
    plotDF1$A_ratio <- with(plotDF1, An_sun/An_sha)
    
    
    ### relative contribution of sun and shaded leaf to total simulated A
    p1 <- ggplot(plotDF1) +
        geom_point(aes(An_can, A_ratio, col=as.factor(canopy)),
                   size=2)+
        theme_linedraw() +
        theme(panel.grid.minor=element_blank(),
              axis.text.x=element_text(size=12),
              ...
              legend.position="none",
              legend.text.align=0)+
        xlab(expression(paste(A[can]* " (" * mu * "mol " * CO[2] * " " * m^-2 * " " * s^-1 * ")")))+
        ylab(expression(paste(A[sha]* " (" * mu * "mol " * CO[2] * " " * m^-2 * " " * s^-1 * ")")))+
        scale_color_manual(name="Canopy",
                           limits=c("12345", "345", "45"),
                           values=c("blue2", "red3", "orange"),
                           labels=c("Full", "T+M", "Top"))#+
