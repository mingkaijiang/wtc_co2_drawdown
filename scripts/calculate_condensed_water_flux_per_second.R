calculate_condensed_water_flux_per_second <- function(myDF) {
    ...
    for (i in unique(myDF.out$chamber)) {
        for (j in unique(myDF.out$canopy)) {
            ...
            for (k in 6:length(t.elapsed)) {
                ...
                m1 <- mean(myDF.out[myDF.out$time_elapsed%in%t.elapsed[(k-5):(k-1)]&myDF.out$chamber==i&myDF.out$canopy==j,"CondWater"], na.rm=T)
                m2 <- mean(myDF.out[myDF.out$time_elapsed%in%t.elapsed[(k-4):(k)]&myDF.out$chamber==i&myDF.out$canopy==j,"CondWater"], na.rm=T)
                
                m3 <- mean(myDF.out[myDF.out$time_elapsed==t.elapsed[k]&myDF.out$chamber==i&myDF.out$canopy==j,"CondWater"], na.rm=T)
                m4 <- mean(myDF.out[myDF.out$time_elapsed==t.elapsed[k-1]&myDF.out$chamber==i&myDF.out$canopy==j,"CondWater"], na.rm=T)
                
                myDF.out[myDF.out$time_elapsed==t.elapsed[k]&myDF.out$chamber==i&myDF.out$canopy==j, "CondWater_smooth"] <- (m2-m1)/
