compare_ACI_results_with_canopy_and_leaf_data <- function(cDF) {
    ...
    #### at the moment, we have VPD data from met data files for canopy CO2 drawdown period,
    ### but, we possibly do not have H2O (i.e. transpiration) data,
    ### which means that we probably can't infer Ci for canopy.
    ### check with Craig.
